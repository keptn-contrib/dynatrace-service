...
func TestRetrieveMetricsFromDashboardDataExplorerTile_Errors(t *testing.T) {
	const testDataFolder = "./testdata/dashboards/data_explorer/tile_title_errors/"

	handler := test.NewFileBasedURLHandler(t)
	handler.AddExact(dynatrace.DashboardsPath+"/"+testDashboardID, testDataFolder+"data-explorer-tile-title-parsing-errors.json")

	// there are 24 tiles - one is ignored due to no slo definition, so 23 assertions
	// TODO 2022-05-06: remove necessity of ordering of assertions -> could be done via `sli-name` and maps
	assertionFuncs := []func(*testing.T, *keptnv2.SLIResult){
		createFailedSLIResultAssertionsFunc("duplicate weight", "duplicate key", "'weight'"),
		createFailedSLIResultAssertionsFunc("duplicate key", "duplicate key", "'key'"),
		// full tile name will be used here, because no SLI name could be extracted
		createFailedSLIResultAssertionsFunc("empty sli name;sli=;pass=<500;pass=<600,<+5%;warning=<800,<+10%;weight=10;key=true;", "sli name is empty"),
		// full tile name will be used here, because no SLI name could be extracted
		createFailedSLIResultAssertionsFunc("whitespace sli name;sli=   ;pass=<500;pass=<600,<+5%;warning=<800,<+10%;weight=10;key=true;", "sli name is empty"),
		createFailedSLIResultAssertionsFunc("invalid pass op", "pass", ">>7"),
		createFailedSLIResultAssertionsFunc("duplicate sli", "duplicate key", "'sli'"),
		createFailedSLIResultAssertionsFunc("invalid pass value", "<5ms"),
		createFailedSLIResultAssertionsFunc("invalid pass missing op", "5"),
		createFailedSLIResultAssertionsFunc("invalid pass wrong type", "no!"),
		createFailedSLIResultAssertionsFunc("invalid warning op", "<<5"),
		createFailedSLIResultAssertionsFunc("invalid warning value", "<5s"),
		createFailedSLIResultAssertionsFunc("invalid warning missing op", "5"),
		createFailedSLIResultAssertionsFunc("invalid warning wrong type", "yes"),
		createFailedSLIResultAssertionsFunc("invalid weight value", "4."),
		createFailedSLIResultAssertionsFunc("invalid weight value percent", "4.%"),
		createFailedSLIResultAssertionsFunc("invalid weight value decimal", "4.5"),
		createFailedSLIResultAssertionsFunc("invalid weight value string", "heavy"),
		createFailedSLIResultAssertionsFunc("invalid key value string", "not a boolean value: no"),
		createFailedSLIResultAssertionsFunc("invalid key value number", "not a boolean value: 3"),
		createFailedSLIResultAssertionsFunc("invalid pass value decimal", "<5."),
		createFailedSLIResultAssertionsFunc("invalid pass value percent", "<5.%"),
		createFailedSLIResultAssertionsFunc("invalid warning value decimal", "<5."),
		createFailedSLIResultAssertionsFunc("invalid warning value percent", "<5.%"),
	}

	rClient := &uploadErrorResourceClientMock{t: t}
	runAndAssertThatDashboardTestIsCorrect(t, testDataExplorerGetSLIEventData, handler, rClient, getSLIFinishedEventFailureAssertionsFunc, assertionFuncs...)
}
